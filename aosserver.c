...
void compose(char sender[1024],char toSub[1024], char toBody[1024], char * emailfile, int n,char *);
char * mail_inbox();
char * showInbox();
...
//compose email
void compose(char sender[1024],char toSub[1024], char toBody[1024], char * emailfile, int n, char *to)
{
	FILE *fp;
	...
	fprintf(fp, "||(UNREAD)||ND||%s||%s||%s||%s||%s||%s||%s\n", sender,firstname,lastname,toSub,toBody,time_send,to);
	fclose(fp);
}
...
void emailfileToAt(FILE *fp, char emailFile[1024], int n )
{
	char *token;
	char line_file[1024];
	int i,j=0,k=0;
	...
	for(i=0;i<n;i++)
	{
		if(fgets(line_file, 1024,(FILE *)fp)!=NULL)
		{
				...
				//token = strtok(NULL, "||");	
				//printf("\n token = %s", token);
				strncpy(email_at[j++], token,strlen(token)+1);
				//}
...
char * mail_inbox()
{
	...
	emailfileToAt(fpcfg,email_File,lines);
...
char * check_email(char buff[LENGTH])
{
	...
	strcat(check_email_buff,"|");
	strcat(check_email_buff, email_at[mail_seq_ND[n-1]]);
	strcat(check_email_buff,"|");
...
void email_reply(char buff[LENGTH])
{
	...
	email_File= emailservice(nameOfFile);	
	printf("%s", email_File);
	compose(current_user,email_subject[n],email_Body,email_File,lines,"NULL");
...
void email_forward(char buff[LENGTH])
{
	...
	for(i=0;i<count ;i++)
	{
		...
		email_File= emailservice(nameOfFile);	
		printf("%s", email_File);
		compose(current_user,email_subject[n],email_body[n],email_File,lines,"NULL");
	}
