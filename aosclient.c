...
int main(int argc, char *argv[])
{
	...
	char * loginDetail, *token_set, *mail_detail,*calendar_detail, *token, *email_forward_content, *email_reply_content, *filter_choice;
	char *cal_detail;
   ...
   while(1)
	{
		...
		while(1)
		{
			...
			if(strcmp(tempBuff,"checkkk") == 0)
			{
				...
				{
					printf("\n Save attachment");
					if(strcmp(att, "NULL") == 0)
					{	
						printf("No attachment to save !!\n");
					}
					else
					{
						printf("Saving attachment to %s\n", att);
						fp=fopen(att, "a");
						fclose(fp);
					}
					memset(sendBuff,'\0',sizeof(sendBuff));
...
char * mail_compose()
{
	...
	char toRecievers[100], toSub[500], toBody[500], toEmail[2048], cu_user[20], attachment[50];
	int len;
	char choice[50];
	printf("\n =====> NEW Email Creation\n");
	...
	{
		printf("Enter the attachment File name:");
		fgets(attachment, sizeof(attachment), stdin);
		len = strlen(attachment);
		if(attachment[len - 1] == '\n')
			attachment[len - 1] = '\0';
	}
	else
		strcpy(attachment, "NULL");

	strcat(attachment, "|");

	strcpy(toEmail, toRecievers);
	strcat(toEmail, toSub);
	strcat(toEmail, toBody);
	strcat(toEmail, attachment); 
